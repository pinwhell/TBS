...
TEST_CASE("Batch Pattern Search Test")
{
	...
	testCase.push_back(TBS::PatternDescBuilder().setPattern("F7 ? 5D C3").Build());
...
TEST_CASE("Test Builder")
{
	TBS::BatchPatternsScanResults results;

	bool foundAll = TBS::LocalFindPatternBatch(
		{ TBS::PatternDescBuilder()
		.setPattern("B8 ? ? ? ? 83 ? ? 83 ? ? BB")
		.setUID("Example")
		.setStartFind(gSample1.data() + 5) // Purposefully starting after the pattern beginning
		.setEndFind(gSample1.end()._Ptr)
		.Build() },
		TBS::VecStart(gSample1),
		TBS::VecEnd(gSample1),
		results
	...
	foundAll = TBS::LocalFindPatternBatch(
		...
		.setPattern("B8 ? ? ? ? 83 ? ? 83 ? ? BB")
		.setUID("Example")
		.setStartFind(gSample1.data() + 3) // Starting right at the pattern beginning (edge case)
		.Build() },
		TBS::VecStart(gSample1),
		TBS::VecEnd(gSample1),
		results
