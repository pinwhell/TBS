...
TEST_CASE("Memory Searching First")
{
	...
	auto result = Memory::SearchFirst(testCase, testCase + sizeof(testCase), toFind);
	CHECK(result == testCase + 7);

	result = Memory::SearchFirst(result + 1, testCase + sizeof(testCase), toFind);
	CHECK(result == testCase + 8);

	result = Memory::SearchFirst(result + 1, testCase + sizeof(testCase), toFind);
	CHECK(result == testCase + 32);
}